...
NOTE: This setting is only required for Raspberry Pi 4 family devices. Raspberry Pi 5 family devices do not require this setting as they have a built-in power button for RPIBOOT mode entry.

WARNING: This value is programmed into OTP (one-time programmable memory) and cannot be changed once set. Ensure you use the same GPIO pin that you will physically connect to ground when entering RPIBOOT mode.

See the link:device-guidance/pi4.adoc[Raspberry Pi 4 Connection Guide] for more information about GPIO configuration.
...
=== RPI_SB_PROVISIONER_FASTBOOT_TRANSPORT
*Optional*

Controls the transport used for the fastboot connection to the target device during provisioning.

* `usb` (default) - Pin the fastboot connection to the device's USB serial for the entire provisioning run. The provisioner does not query the device for its IP addresses and does not probe TCP reachability.
* `auto` - Prefer an Ethernet (TCP) connection when the device is reachable over the network, falling back to USB otherwise. This accelerates image transfer, but selects the target purely by IP address.

NOTE: When provisioning multiple devices on the same network, use `usb`. Fastboot over TCP binds to an IP address with no association to the device serial, so with several targets on one network the connection can be routed to an unintended device, making target selection unstable. `auto` is only recommended when provisioning a single device at a time and accelerated image transfer over Ethernet is desired.
